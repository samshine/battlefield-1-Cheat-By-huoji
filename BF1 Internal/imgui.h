...
void   LoadSettings(); //exposed for external call
void   MarkSettingsDirty();  //exposed for external call
...
namespace ImGui
{
    ...
    IMGUI_API unsigned short        InputFloat(const char* label, float* v, float step = 0.0f, float step_fast = 0.0f, int decimal_precision = -1, ImGuiInputTextFlags extra_flags = 0);
	...
	static inline bool      IsCursorHover(ImVec4 _rect)
	{
		ImVec2 MP = GetMousePos();
		if (MP.x >= _rect.x && MP.x <= _rect.x + _rect.z &&
			MP.y >= _rect.y && MP.y <= _rect.y + _rect.w)
			return true;
		return false;
	}
#endif
